...
class OrbitNavigator:
    def __init__(self, name = "Drone1", radius = 2, altitude = 10, speed = 2, iterations = 1, center = [1,0], snapshots = None):
        self.name = name
        self.radius = radius
        ...
        self.home = self.client.getMultirotorState(vehicle_name=self.name).kinematics_estimated.position
    ...
    def start(self):
        ...
        start = self.client.getMultirotorState(vehicle_name=self.name).kinematics_estimated.position
        landed = self.client.getMultirotorState(vehicle_name=self.name).landed_state
        ...
        while count < self.iterations:
            ...
            self.client.moveByVelocityZAsync(vx, vy, z, 1, airsim.DrivetrainType.MaxDegreeOfFreedom, airsim.YawMode(False, camera_heading),vehicle_name=self.name)


        self.client.moveToPositionAsync(start.x_val, start.y_val, z, 2,vehicle_name=self.name).join()

        if self.takeoff:            
            # if we did the takeoff then also do the landing.
            if z < self.home.z_val:
                print("descending")
                self.client.moveToPositionAsync(start.x_val, start.y_val, self.home.z_val - 5, 2,vehicle_name=self.name).join()
    ...
    def take_snapshot(self):
        ...
        self.client.moveToPositionAsync(pos.x_val, pos.y_val, self.z, 0.5, 10, airsim.DrivetrainType.MaxDegreeOfFreedom, 
            airsim.YawMode(False, self.camera_heading), vehicle_name=self.name).join()
        responses = self.client.simGetImages([airsim.ImageRequest(1, airsim.ImageType.Scene), \
                                              airsim.ImageRequest(1, airsim.ImageType.Segmentation)],vehicle_name=self.name) #scene vision image in png format
        for j, r in enumerate(responses):
            filename = "snapshots/photo_" + str(self.name) + "_" + str(self.snapshot_index) + "_" + str(j)
            airsim.write_file(os.path.normpath(filename + '.png'), r.image_data_uint8)        
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--name",help="name of the drone to control", default="Drone1")
    ...
    arg_parser.add_argument("--cx", type=float, help="x direction vector pointing to center of orbit from current starting position default 1", default=1)
    arg_parser.add_argument("--cy", type=float, help="y direction vector pointing to center of orbit from current starting position default 1", default=1)
    ...
    nav = OrbitNavigator(args.name, args.radius, args.altitude, args.speed, args.iterations, [args.cx, args.cy], args.snapshots)
    nav.start()
